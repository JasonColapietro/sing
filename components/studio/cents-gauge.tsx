...
export function CentsGauge({ cents }: { cents: number | null }) {
  ...
  return (
    ...
    >
      {/* dial face */}
      <path d={arc(-50, 50, 95)} fill="none" stroke={PANEL2} strokeWidth={14} strokeLinecap="round" />
      <path d={arc(-50, 50, 95)} fill="none" stroke={LINE} strokeWidth={1} />
      {/* green zone ±15 cents */}
      <path d={arc(-15, 15, 95)} fill="none" stroke={OK} strokeOpacity={0.55} strokeWidth={14} />
      {/* ticks */}
      {TICKS.map((t) => {
      ...
      >
        <line x1={CX} y1={CY + 10} x2={CX} y2={CY - 88} stroke={AMBER} strokeWidth={2.5} strokeLinecap="round" />
      </g>
      {/* pivot */}
      <circle cx={CX} cy={CY} r={7} fill={PANEL2} stroke={LINE2} strokeWidth={1.5} />
      <circle cx={CX} cy={CY} r={2.5} fill={AMBER} opacity={active ? 1 : 0.3} />
    </svg>
  );
}
